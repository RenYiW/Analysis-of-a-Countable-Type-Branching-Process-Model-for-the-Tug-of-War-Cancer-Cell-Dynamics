...
#-------------------------------------------------------------------------------
# Cell specs
mu <- 0.006 # Driver mutation rate
nu <- 0.09 # Passenger mutation rate
...
N = 20*L # Decide the number of initial types
...
type_vs_ext_P2 <- as.data.frame(cbind(ini_types, ext_prob))

ggplot(type_vs_ext_P2, aes(x=ini_types, y=ext_prob)) + 
  geom_point() + xlab("Initial Types")+ylab("Extinction Probability")
ggsave("ext_prob_P2.png")

write.csv(type_vs_ext_P2,"type_vs_ext_P2.csv")
