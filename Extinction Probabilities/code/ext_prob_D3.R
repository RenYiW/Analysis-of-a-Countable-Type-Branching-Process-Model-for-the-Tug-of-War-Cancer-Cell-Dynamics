...
N = 10*L # Decide the number of initial types
...
type_vs_ext_D3 <- as.data.frame(cbind(ini_types, ext_prob))

ggplot(type_vs_ext_D3, aes(x=ini_types, y=ext_prob)) + 
  geom_point() + xlab("Initial Types")+ylab("Extinction Probability")
ggsave("ext_prob_D3.png")

write.csv(type_vs_ext_D3,"type_vs_ext_D3.csv")
