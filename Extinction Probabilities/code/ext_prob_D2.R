...
#-------------------------------------------------------------------------------
# Simulation specs
n = 150 # >= t+nL+1 or <= t-nL-1 are taboo types
...
type_vs_ext_D2 <- as.data.frame(cbind(ini_types, ext_prob))

ggplot(type_vs_ext_D2, aes(x=ini_types, y=ext_prob)) + 
  geom_point() + xlab("Initial Types")+ylab("Extinction Probability")
ggsave("ext_prob_D2.png")

write.csv(type_vs_ext_D2,"type_vs_ext_D2.csv")
